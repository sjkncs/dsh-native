...
> **注意：工具脚本未随附**：本文档引用到的 `tools/*` 路径（如 `tools/latex`、`tools/docx`、`tools/figure/scripts`、`tools/paper_search` 等）来自参考仓库的**工具脚本**，本预设**仅随附方法论文档，未包含这些脚本**。遇到文档中的此类命令时：按正文描述的方法自行实现等价脚本，或由团队在共享工作区另行接入对应工具；**不要假设这些脚本存在于本机**。方法规范本身仍然有效。
