...
export const SORT_BY = { relevance: 'relevance', lastUpdatedDate: 'lastUpdatedDate', submittedDate: 'submittedDate' } as const
...
export function parseArxivFeed(xml: string): { papers: Paper[]; total: number } {
  ...
  while ((m = entryRe.exec(xml)) !== null) {
    ...
    while ((am = authRe.exec(e)) !== null) authors.push(parseXmlText(am[1].trim()))
    ...
    const externalIds: Record<string, string> = { ArXiv: id }
...
export async function searchArxiv(opts: {
  ...
  dateFrom?: string
  signal?: AbortSignal
}): Promise<{ papers: Paper[]; total: number; nextOffset?: number }> {
  ...
  if (opts.dateFrom) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(opts.dateFrom)) throw new Error('date_from must be YYYY-MM-DD')
