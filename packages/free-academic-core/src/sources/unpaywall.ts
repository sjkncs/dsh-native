...
function toOaLocation(raw: unknown): UnpaywallOaLocation | undefined {
 ...
 */
export async function lookupUnpaywallOa(
  doi: string,
  opts: { email?: string; signal?: AbortSignal } = {},
  ...
  const res = await fetchJson<Record<string, unknown>>(url, { signal: opts.signal })
  if (!res.ok) return undefined // includes HTTP 404 (paper not in Unpaywall)
  if (res.data.is_oa !== true) return undefined
  ...
  const oaLocations = rawLocations
    ...
    .filter((loc): loc is UnpaywallOaLocation => loc !== undefined)
  return compact({
    doi: norm,
    isOa: true,
 ...
 */
export async function downloadUnpaywallPdf(
  doi: string,
  opts: { email?: string; signal?: AbortSignal } = {},
