...
export function mapRxivItem(raw: Record<string, unknown>): Paper {
  ...
  const rawServer = (raw.server ?? raw.preprint_server) as unknown
  const server: 'medrxiv' | 'biorxiv' = rawServer === 'medRxiv' ? 'medrxiv' : 'biorxiv'
  const authors = Array.isArray(raw.authors) ? (raw.authors as string[]).filter(Boolean) : []
...
export async function searchRxiv(opts: {
  ...
  for (let page = 0; page < maxPages && collected.length < wanted; page++) {
    ...
    const res = await fetchJson<{ collection?: Array<Record<string, unknown>>; messages?: unknown[] }>(url, { signal: opts.signal })
    if (!res.ok) throw new Error(`bioRxiv/medRxiv search failed: ${res.error}`)
    const items = res.data.collection ?? []
    if (items.length === 0) break
    for (const item of items) {
      const title = typeof item.title === 'string' ? item.title : ''
      const abstract = typeof item.abstract === 'string' ? item.abstract : ''
      if (`${title} ${abstract}`.toLowerCase().includes(q)) collected.push(mapRxivItem(item))
    }
