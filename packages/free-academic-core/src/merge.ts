 ...
 */
export function mergePaperLists(lists: Paper[][]): Paper[] {
  ...
  for (const list of lists) {
    for (const p of list) {
      ...
      if (existing === undefined || better(p, existing)) papers.set(p.id, p)
    }
  }
  ...
  for (const [id, p] of papers) {
    for (const key of identityKeys(p)) {
      ...
      if (owner !== undefined) union(owner, id)
  ...
  for (const list of lists) {
    ...
    for (const p of list) roots.add(find(p.id))
    for (const root of roots) hitCount.set(root, (hitCount.get(root) ?? 0) + 1)
  }
...
function better(a: Paper, b: Paper): boolean {
