 ...
 */
export async function downloadSciHubPdf(doi, opts = {}) {
    ...
    for (const mirror of mirrors) {
        ...
        if (!page.ok)
            continue; // 403 / Cloudflare / 404 → skip this mirror.
        ...
        if (!bin.ok)
            continue;
        if (!isPdfPayload(bin.contentType, bin.data))
            continue;
        return { bytes: bin.data, finalUrl: bin.finalUrl, contentType: bin.contentType };
    }
    return undefined;
}
//# sourceMappingURL=sci-hub.js.map