...
export async function searchPapers(opts) {
    const sources = opts.sources ?? ['arxiv', 'semantic-scholar', 'pubmed'];
    const limit = Math.min(Math.max(1, opts.maxResults ?? 10), 50);
    const perSource = Math.max(5, Math.ceil(limit * 1.5));
    const tasks = [];
    if (sources.includes('arxiv'))
        tasks.push(searchArxiv({ query: opts.query, maxResults: perSource, signal: opts.signal }).then((r) => r.papers));
    if (sources.includes('semantic-scholar'))
        tasks.push(searchSemanticScholar({ query: opts.query, maxResults: perSource, signal: opts.signal }));
    if (sources.includes('pubmed'))
        tasks.push(searchPubmed({ query: opts.query, maxResults: perSource, signal: opts.signal }));
    if (sources.includes('biorxiv'))
        tasks.push(searchRxiv({ query: opts.query, server: 'biorxiv', maxResults: perSource, signal: opts.signal }));
    if (sources.includes('medrxiv'))
        tasks.push(searchRxiv({ query: opts.query, server: 'medrxiv', maxResults: perSource, signal: opts.signal }));
    const settled = await Promise.allSettled(tasks);
    const results = settled.filter((r) => r.status === 'fulfilled').map((r) => r.value);
    let merged = mergePaperLists(results);
    if (opts.yearFrom !== undefined)
        merged = merged.filter((p) => (p.year ?? 0) >= (opts.yearFrom ?? 0));
    if (opts.yearTo !== undefined)
        merged = merged.filter((p) => (p.year ?? 9999) <= (opts.yearTo ?? 9999));
