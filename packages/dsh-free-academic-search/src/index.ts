...
import {
  ...
  sliceText,
} from 'free-academic-core'
import type { Paper, Source } from 'free-academic-core'
...
function enabled(v: boolean | undefined): boolean {
  return v !== false
}
...
const SOURCES = ['semantic-scholar', 'pubmed', 'arxiv', 'biorxiv', 'medrxiv', 'doi'] as const

/** Every `source` value the DOI tools (`download_by_doi` / `read_by_doi`) can emit. */
const DOI_SOURCES = ['doi', 'arxiv', 'unpaywall', 'scihub'] as const
...
/** Model-facing text content block. */
type TextBlock = { type: 'text'; text: string }

/** Structural view of a paper used by the renderers (subset of core's `Paper`). */
type PaperLike = {
  id: string
  title: string
  authors: string[]
...
/** The list-page value the per-platform search tools return. */
type PaperListPage = {
  source: string
  query: string
  total: number
...
/** Author profile value returned by `get_semantic_author` (every field optional). */
type AuthorProfile = {
  authorId?: string
  name?: string
  hIndex?: number
...
function idLabel(source: string): string {
  switch (source) {
...
function formatPaper(paper: PaperLike, index?: number): string {
  const lines: string[] = []
...
function renderPaperList(label: string, value: PaperListPage): TextBlock[] {
  if (value.papers.length === 0) return [{ type: 'text', text: `No ${label} results for "${value.query}".` }]
  const parts: string[] = []
...
function renderPaperSingle(label: string, paper: PaperLike): TextBlock[] {
  return [{ type: 'text', text: `${label} record:\n\n${formatPaper(paper)}` }]
}

/** Model-facing text for a bare list of papers under a caller-supplied heading. */
function renderPaperArray(label: string, heading: string, papers: PaperLike[]): TextBlock[] {
  if (papers.length === 0) return [{ type: 'text', text: `No ${label} results.` }]
...
function renderAuthor(value: AuthorProfile): TextBlock[] {
  const lines: string[] = ['Semantic Scholar author:']
  const info: string[] = []
  if (value.name !== undefined) info.push(value.name)
  if (value.authorId !== undefined) info.push(`authorId: ${value.authorId}`)
  if (value.hIndex !== undefined) info.push(`h-index: ${value.hIndex}`)
  if (value.paperCount !== undefined) info.push(`${value.paperCount} papers`)
  if (value.citationCount !== undefined) info.push(`${value.citationCount} citations`)
  ...
  if (value.url !== undefined) lines.push(`Profile: ${value.url}`)
  return [{ type: 'text', text: lines.join('\n') }]
}

/** Model-facing text for one rxiv full-text slice (`read_biorxiv_paper` / `read_medrxiv_paper`). */
function renderRxivRead(value: { doi: string; text: string; offset: number; hasMore: boolean; totalPages?: number }): TextBlock[] {
...
function renderUnifiedSearch(value: { query: string; sources: string[]; total: number; papers: PaperLike[]; truncated: boolean }): TextBlock[] {
  const label = value.sources.map((s) => SOURCE_LABEL[s] ?? s).join(', ')
  if (value.papers.length === 0) return [{ type: 'text', text: `No papers found for "${value.query}" across ${label}.` }]
  const parts: string[] = []
...
function renderRead(value: { id: string; pdfUrl: string; pages?: number; totalChars: number; offset: number; text: string; truncated: boolean; nextOffset?: number }): TextBlock[] {
  const head: string[] = [`Full text of ${value.id}`]
...
function sliceReadValue(fullText: string, offset: number | undefined, maxChars: number | undefined) {
...
/** Result of {@link downloadWithFallback}: the bytes plus where they actually came from. */
type DownloadWithSource = {
  bytes: Uint8Array
  finalUrl: string
  contentType: string
  /** The normalized identifier that was resolved (a DOI for the doi route, an arXiv id otherwise). */
  identifier: string
  /** 'doi' when the PDF came through doi.org, 'arxiv' when an arXiv preprint was used, 'unpaywall' / 'scihub' for the opt-in fallback layers. */
  source: 'doi' | 'arxiv' | 'unpaywall' | 'scihub'
  /** Set when a fallback route served a paywalled DOI. */
  note?: string
}

/** Options shared by the DOI resolution fallback chain: the cooperative signal plus the validated plugin config. */
...
function isLandingPageError(err: unknown): boolean {
...
function arxivIdFromDoi(doi: string): string | undefined {
 ...
 */
async function arxivIdViaSemanticScholar(doi: string, signal?: AbortSignal): Promise<string | undefined> {
  try {
...
async function downloadArxivPdf(id: string, signal?: AbortSignal): Promise<{ bytes: Uint8Array; finalUrl: string; contentType: string }> {
  const res = await fetchBinary(arxivPdfUrl(id), { signal })
  if (!res.ok) throw new Error(`arXiv PDF download failed: ${res.error}`)
 ...
 */
async function downloadWithFallback(input: string, opts: FallbackOptions): Promise<DownloadWithSource> {
...
// ---- /free-academic slash command (Task 26) ----

/**
 * Minimal structural view of the lazily-injected `commands` service (the
 * app's `@deepseek-ai/dsh-commands` registry). We deliberately do NOT import
 * that package: `commands` is optional under headless, so the command only
 * registers through `ctx.inject(['commands'], ...)` when the service exists.
 * The runtime shape is pinned here to match the app-shipped registry
 * (`normalizeDefinition` accepts `{ name, description, input?, handler }`;
 * `normalizeResult` accepts `{ kind: 'success'|'error', text }`).
 */
 ...
 * `apply` below.
 */
const TOOL_USAGE: ReadonlyArray<readonly [string, string]> = [
...
function renderCommandStatus(config: PluginConfig): string {
  const toggles: Array<readonly [string, string]> = [
...
function parseSearchFlags(query: string): { maxResults?: number; yearFrom?: number; yearTo?: number } {
  let maxResults: number | undefined
  let yearFrom: number | undefined
  let yearTo: number | undefined
  const nMatch = /(?:^|\s)--n\s+(\d+)/u.exec(query)
  if (nMatch !== null) maxResults = Math.min(20, Math.max(1, Number(nMatch[1])))
...
function stripSearchFlags(query: string): string {
...
function renderCommandResults(query: string, papers: Paper[]): string {
  if (papers.length === 0) {
...
export function apply(ctx: Context, config: PluginConfig): void {
  if (enabled(config.arxiv)) {
    ...
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return { id, pdfUrl: pdf.finalUrl, pages, source: 'arxiv' as const, ...sliceReadValue(text, args.offset, args.max_chars) }
      },
    }))
  }
  ...
  if (enabled(config.semanticScholar)) {
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return { source: 'semantic-scholar' as const, query: args.query, total: papers.length, papers, truncated: false }
      },
    }))
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return {
          ...
          ...(Array.isArray(raw.affiliations)
            ? { affiliations: raw.affiliations.filter((a): a is string => typeof a === 'string') }
            : {}),
  ...
  if (enabled(config.pubmed)) {
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return { source: 'pubmed' as const, query: args.query, total: papers.length, papers, truncated: false }
      },
    }))
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        if (!paper) return { found: false, message: `No PubMed record found for PMID ${args.pmid}.` }
  ...
  if (enabled(config.biorxiv)) {
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return { source: 'biorxiv' as const, query: args.query, total: papers.length, papers, truncated: false }
      },
    }))
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return { doi, finalUrl: pdf.finalUrl, contentType: pdf.contentType, size: pdf.bytes.length, source: 'biorxiv' as const }
      },
    }))
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return { doi, text: slice.text, offset: slice.offset - slice.text.length, hasMore: slice.hasMore, totalPages: pages, source: 'biorxiv' as const }
      },
    }))
  }
  ...
  if (enabled(config.medrxiv)) {
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return { source: 'medrxiv' as const, query: args.query, total: papers.length, papers, truncated: false }
      },
    }))
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        const doi = normalizeDoi(args.doi)
        const pdf = await downloadPdfByDoi(doi, { signal: exec.signal })
        return { doi, finalUrl: pdf.finalUrl, contentType: pdf.contentType, size: pdf.bytes.length, source: 'medrxiv' as const }
      },
    }))
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        if (text.trim().length === 0) throw new Error('the PDF downloaded but no readable text could be extracted (scanned or image-only PDF)')
        const slice = sliceText(text, args.offset ?? 0, args.max_chars ?? DEFAULT_MAX_CHARS)
        return { doi, text: slice.text, offset: slice.offset - slice.text.length, hasMore: slice.hasMore, totalPages: pages, source: 'medrxiv' as const }
      },
    }))
  }
  ...
  if (enabled(config.unified)) {
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        const sources: Source[] = args.sources !== undefined && args.sources.length > 0
          ? args.sources
  ...
  if (enabled(config.command)) {
    ctx.inject(['commands'], (commandCtx) => {
      const commands = (commandCtx as unknown as { commands: { register: (definition: CommandDefinition) => unknown } }).commands
      commands.register({
        name: 'free-academic',
