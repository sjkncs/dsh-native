...
/** 侧栏图标备选集（旧版 emoji 存储键，界面统一按 SVG 渲染）：布局/快捷方式/入驻项目的图标，点击可换（首项为布局默认） */
const EMOJI_SET = ['\u{1F9F1}', '\u{1F3E0}', '\u{1F393}', '\u{1F697}', '\u{2708}\u{FE0F}', '\u{1F30D}', '\u{1F3E5}', '\u{1F4DA}', '\u{270F}\u{FE0F}', '\u{2699}\u{FE0F}', '\u{1F3A8}', '\u{1F3AE}', '\u{1F30F}', '\u{1F4D0}', '\u{1F9EA}', '\u{1F916}', '\u{1F4E6}', '\u{1F4AC}']
...
function presetThumb(defId: string) {
  const cell = (chat: boolean, key: string) => (
    <span key={key} className={'dsh-wt_thumbCell' + (chat ? ' dsh-wt_thumbChat' : '')}>{chat ? <Icon name="chat" /> : ''}</span>
  )
  if (defId === '2h') {
...
const CONSOLE_ICON = '\u{1F5A5}\u{FE0F}'
...
const KNOWLEDGE_PACK = [
  ...
  '- 产物一律在「工作台窗内」经 iframe 渲染，与操作系统无关：严禁用 Invoke-Item / start / 双击 / 调用系统默认程序去打开产物，',
  '  也不要让用户去设置 Windows 文件关联或改注册表。窗口空白/打不开时先核对 widget-result.json 的 window/path/kind，',
  '  再在浏览器直接访问 http://127.0.0.1:3080/api/worktable/site/<产物目录>/<产物文件> 验证能否 200 渲染。',
  '- 表格/数据文件（.csv/.tsv/.json）可直接用 kind:"data" 挂载，工作台用「数据速览」窗渲染（表格 + 列统计 + 内联 SVG 图表），',
  '  只想展示数据时不必手写 HTML。',
...
function buildWindowTaskText(projectId: string, projectName: string, windowLabel: string, requirement: string, folder: string | null, mode: 'new' | 'send'): string {
  ...
  const lines = [
    ...
    '   - 纯数据展示（结果表 / 统计 / 对比）→ 生成 .csv/.tsv/.json 放进项目文件夹，清单里用 kind:"data" 挂载（数据速览窗自动渲染表格+统计+图表）；',
    ...
    '6. 完成后写入「产物清单」文件：在项目文件夹里创建 widget-result.json。挂载单个窗口时写单对象 {\"window\":\"' + win + '\",\"path\":\"产物相对路径\",\"kind\":\"html\"}；一次挂载多个窗口（如窗口1 + 窗口2）时写 JSON 数组，每个元素为上述单对象（可引用多个产物文件）。kind 可选 html（本地页面，path 相对项目文件夹）/ url（外部链接，path 为完整 URL）/ file（其他文件，path 相对项目文件夹）/ data（表格数据 .csv/.tsv/.json，渲染成「数据速览」窗：表格+列统计+图表，path 相对项目文件夹）。写完这个文件，工作台会自动把产物挂载进清单对应的各个窗口并锁定保存（数秒内轮询落位），用户无需手动操作；用户下次打开工作台时各窗口直接显示产物，不会丢失或重置。',
    KNOWLEDGE_PACK,
  ]
  return lines.join('\n')
}
...
/** 产物清单 → 窗口内容（html=目录级托管 iframe / url=外链 iframe / file=文件预览 / data=数据速览窗） */
function buildMountContent(folder: string, d: any): any {
  ...
  if (d?.kind === 'data') return { kind: 'builtin', type: 'data', url: full }
...
function buildCustomLayoutPrompt(req: string): string {
  return [
    ...
    '6. 在 presetThumb(defId) 里为新 id 增加一个缩略图分支（用 dsh-wt_thumb / dsh-wt_thumbCol / dsh-wt_thumbRow / cell()；聊天窗用 cell(true) 的蓝色对话格）。',
  ...
  // 挂载轮询：自愈扫挂只在组件挂载 / 项目文件夹变化时跑一次；若 agent 在页面加载后才写出
  //   widget-result.json，而「绑定会话完成」这一发事件又恰好丢失（组件当时未挂载 / 状态竞态），
  //   产物就一直挂不进窗口，直到用户手动整页刷新。这里每 5 秒巡检一遍清单文件兜底——
  //   指纹去重（已挂载 / 已以同内容暂存则跳过），命中才真正落位，平时开销只有一次小文件读取。
  useEffect(() => {
    let cancelled = false
    const timer = window.setInterval(async () => {
      const folders = projectsRef.current.projects.folders ?? {}
      for (const pid of Object.keys(folders)) {
        if (cancelled) return
        const folder = folders[pid]
        if (!folder) continue
        try {
          ...
          if (cancelled) return
          if (mountedWidgetRef.current[pid] === raw) continue
          const pend = pendingMountRef.current[pid]
          if (pend && pend.fingerprint === raw) continue
          await applyWidgetManifest(pid, raw)
        } catch { /* 单项目失败不影响其余 */ }
      }
    }, 5000)
    ...
    <div ref={rootRef} className={'dsh-wt_section' + (isFloat ? ' dsh-wt_float' : '')} style={isFloat ? floatStyle : dockedStyle}>
        ...
        <div className="dsh-wt_menu dsh-wt_add dsh-wt_pop" style={{ position: 'fixed', left: popLeft, top: popTop, width: 360, zIndex: 80 }}>
          ...
          <div className="dsh-wt_presets">
            ...
            <button type="button" className="dsh-wt_preset dsh-wt_presetAdd" title={t('customLayout.addTitle')}
              ...
              <span className="dsh-wt_presetAddIcon" aria-hidden><Icon name="plus" /></span>
            ...
            <div className="dsh-wt_iconGrid">
              {EMOJI_SET.map((em) => {
                const cur = iconPick.kind === 'layout'
                  ? (projects.layouts.find((l) => l.id === iconPick.id)?.icon ?? '\u{1F9F1}')
                  : iconPick.kind === 'shortcut'
                    ? (projects.shortcuts.find((s) => s.id === iconPick.id)?.icon ?? '\u{1F517}')
                    : (projects.iconOverrides[iconPick.id] ?? metas[iconPick.id]?.icon ?? '\u{1F4E6}')
                return (
                  <button
                    key={em}
                    type="button"
        ...
        <div ref={settingsRef} className="dsh-wt_manage dsh-wt_pop dsh-wt_settings" style={{ position: 'fixed', left: popLeft, top: settingsTop ?? popTop, width: 280, zIndex: 80 }}>
            ...
            <div className="dsh-wt_updateCard">
              ...
              <div className="dsh-wt_updateBtns">
                <button type="button" className="dsh-wt_updateBtn dsh-wt_updateBtnCopy" onClick={() => void copyUpgradeAi()}>
                  {updateCopied ? <><Icon name="check" /> {t('update.copied')}</> : <>{ICON_SPARK} {t('update.copyAi')}</>}
                </button>
              ...
              >
                ...
                {layout
                  ? <span
                    ...
                    >{<Icon name={layout.icon ?? '\u{1F9F1}'} />}</span>
                  : <span
                      className="dsh-wt_manageIcon dsh-wt_iconPick"
                      role="button"
                      tabIndex={0}
                    ...
                    >{<Icon name={projects.iconOverrides[id] ?? meta?.icon ?? '\u{1F4E6}'} />}</span>}
                ...
                <button type="button" className="dsh-wt_manageBtn" title={t('manage.changeView')} onClick={() => setViewPickFor(id)}><Icon name="puzzle" /></button>
                <button
                  type="button"
          ...
          {projects.shortcuts.map((s) => (
            <div key={s.id} className="dsh-wt_manageRow dsh-wt_manageRowSc">
              <span className="dsh-wt_manageGrip" aria-hidden><Icon name="link" /></span>
              <span
                className="dsh-wt_manageIcon dsh-wt_iconPick"
                role="button"
                tabIndex={0}
      ...
      {bindPick && (
        <div className="dsh-wt_menu dsh-wt_pop dsh-wt_bindPop" style={{ position: 'fixed', left: bindPick.x, top: bindPick.y, width: 280, zIndex: 84 }}>
          {/* 项目文件夹框（格式基准）：第一行图标+标题，第二行路径；可随时更改 */}
          <div className="dsh-wt_bindFolderBox">
            <div className="dsh-wt_bindFolderRow">
              <span className="dsh-wt_bindFolderLabel"><Icon name="folder" /> {t('bind.folder')}</span>
          ...
          {/* 绑定对话框（与项目文件夹同格式）：第一行图标+标题（最右解绑）；第二行 分组 | 对话名（点击右侧弹列表，再点反选收起）；框内下方：说明 */}
          <div className="dsh-wt_bindFolderBox">
            <div className="dsh-wt_bindFolderRow">
              <span className="dsh-wt_bindFolderLabel"><Icon name="chat" /> {t('bind.title')}</span>
            ...
            {(() => {
              ...
              return (
                <button type="button" className="dsh-wt_bindConvRow" title={t('bind.tipBound', { name: info.title })} onClick={() => setBindListOpen((v) => !v)}>
                  <span className="dsh-wt_bindFolder" title={info.folder}><Icon name="folderOpen" /> {info.folder}</span>
        ...
        <div className="dsh-wt_menu dsh-wt_pop dsh-wt_consoleBindPop" style={{ position: 'fixed', left: consoleBind.x, top: consoleBind.y, width: 560, zIndex: 86 }}>
          <div className="dsh-wt_consoleBindCols">
            <div className="dsh-wt_consoleBindCol">
              <span className="dsh-wt_consoleBindLabel"><Icon name="plus" /> {t('console.joinExisting')}</span>
            ...
            <div className="dsh-wt_consoleBindCol dsh-wt_consoleBindColNew">
              <span className="dsh-wt_consoleBindLabel"><Icon name="spark" /> {t('console.newConv')}</span>
      ...
      {customOpen && (
        <div className="dsh-wt_menu dsh-wt_pop" style={{ position: 'fixed', left: popLeft, top: popTop, width: 340, zIndex: 84 }}>
          <span className="dsh-wt_menuLabel"><Icon name="spark" /> {t('customLayout.title')}</span>
          <textarea
          ...
          {copiedToast && (
            <p className={'dsh-wt_customLayoutToast' + (copiedToast === 'fail' ? ' dsh-wt_customLayoutToastFail' : '')}>
              {copiedToast === 'ok' ? <><Icon name="check" /> {t('customLayout.copied')}</> : <><Icon name="warning" /> {t('customLayout.copyFail')}</>}
            </p>
          )}
        </div>
      )}
      ...
      {requestDelete && (
        <div className="dsh-wt_confirm" role="alertdialog">
          <div className="dsh-wt_confirmTitle"><Icon name="warning" /> {t('confirm.title')}</div>
      ...
      <div className="dsh-wt_projects" data-managing={viewOptionsOpen ? 'true' : undefined}>
        ...
        >
          <span className="dsh-wt_layoutIcon"><Icon name={CONSOLE_ICON} /></span>
